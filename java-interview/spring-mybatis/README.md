...
2. 内存泄漏 vs OOM
3. Java 常见内存泄漏场景
4. GC 基础算法完整名称：标记清除 / 复制 / 标记整理 / 分代收集
5. JVM 五大运行时内存区域，重点固定“本地方法栈 / 方法区”术语
6. 年轻代结构：Eden + S0 + S1
7. GC Roots 延迟复测
8. 强引用 / 软引用 / 弱引用 / 虚引用延迟复测
...
### 已明显趋稳
- 对象晋升老年代：连续多轮无提示答对，已经属于 JVM 主线中的较稳定知识点
- GC Roots：从 Day56/57 几乎完全不会，到 Day59 无提示完整说出“栈、静态、常量、JNI / Native”
- 四种引用：从强引用、虚引用混淆，进步到 Day59 基本完整输出
- Minor GC vs Full GC：已能主动区分年轻代回收与更大范围回收

### 正在从“知道”进入“理解”
- Survivor 不只是记住 S0 / S1，而是能说出“为了配合复制算法”
- 两个 Survivor 轮流作为 From / To 区的设计开始形成理解
- Full GC 为什么通常更慢：范围大、处理对象更多，并能开始和 STW 联系起来
- JVM 对象生命周期已经形成主线：`new 对象 → Eden → Minor GC → Survivor → 多次存活 → 晋升老年代`

### 有进步但仍不稳定
- 年轻代结构：Day59 能想起 Eden 和 Survivor，但 S0 / S1 仍未完全稳定
- JVM 五大区域：5 个内容都能想起，但“本地方法栈 / 方法区”的术语仍会说错
- GC 基础算法：复制算法较稳，但标记清除、标记整理仍会混在一起，分代收集容易漏

### Day59 新暴露的问题
- STW：全新概念，当前只建立“GC 某些阶段暂停用户线程”这一层
- 内存泄漏 vs OOM：会直接混为一谈，需要优先建立概念边界
- Java 常见内存泄漏场景：尚不能主动输出
- 面试表达：有时题目问“为什么”，回答却把所有相关场景全部列出；后续要求“先一句话正面回答，再补充扩展”

### Day59 自我成长方向

下一阶段不只追求“知道是什么”，而要逐步建立：

> 为什么会触发 → JVM 做了什么 → 对性能有什么影响 → 线上问题怎么定位。

后续 JVM 会逐步加入 GC 日志、OOM 排查、内存泄漏定位以及 jmap / jstack / MAT 等工程化问题。

# Java 面试训练 Day 08～Day 12 复习整理

> 目标：把 Day 8～Day 12 的 MyBatis、Spring Boot、Spring Batch 高频题整理成可复习、可背诵、可回炉的 Markdown 笔记。
...
3. Mapper 靠动态代理，SQL 靠 `namespace + 方法名` 定位。
4. 一级缓存默认开启，SqlSession 级别；二级缓存默认不开，Mapper 级别，实际项目谨慎使用。
5. PageHelper = MyBatis 插件拦截 SQL + 改写分页 SQL。
6. 小批量可以用 `foreach`，大批量可以用 `ExecutorType.BATCH`，数据多要分批提交。
7. 一对一、一对多：`left join` 负责查数据，`association` / `collection` 负责封装对象。
8. `resultType` 自动映射，`resultMap` 手动映射；简单场景用 `resultType`，复杂映射用 `resultMap`。

---

# Day 09：MyBatis 回炉 + Spring Boot 基础/实战
...
1. 请求链路：`DispatcherServlet → Controller → Service → 事务 → Mapper → MyBatis → MySQL → JSON 返回`。
2. 事务放 Service 层，是为了控制整个业务流程的一致性。
3. 事务失效：没走代理、方法不符合、异常没抛出、对象不归 Spring 管。
4. `@Transactional` 默认 `RuntimeException` 回滚，普通 `Exception` 不回滚，需要配置 `rollbackFor`。
...
# Day 10：周末强化，MyBatis + Spring Boot 回炉，加练实战题
...
问题：把 StringBuilder 和 StringBuffer 的线程安全说反了。

保命句：String 不可变；StringBuilder 可变、线程不安全、性能高；StringBuffer 可变、线程安全、性能低一些。

## HashMap 底层结构和 put 流程

问题：只说了 hash，没说完整 put 流程。

保命句：算 hash，定位下标，没冲突直接放，有冲突比较 key，相同覆盖，不同挂链表或红黑树，超过阈值扩容。

## ConcurrentHashMap 为什么线程安全

问题：答成了 HashMap 为什么线程不安全。

保命句：ConcurrentHashMap 通过减小锁粒度保证线程安全，JDK 1.7 用 Segment 分段锁，JDK 1.8 用 CAS + synchronized。

## Spring Bean 作用域

问题：漏了 prototype，作用域名称不准。

保命句：singleton 默认单例，prototype 每次新建，request 一次请求一个，session 一次会话一个。

## Spring Batch 和普通定时任务区别

问题：只说了 Batch 是大数据量处理，没说定时任务区别。

保命句：普通定时任务负责定时触发；Spring Batch 负责大批量、分步骤、分批处理，并支持失败重试、跳过和重启。

## JobRepository 和 JobLauncher

问题：只知道 JobRepository，不知道 JobLauncher。

保命句：JobLauncher 负责启动 Job，JobRepository 负责记录 Job 和 Step 的执行状态，JobParameters 表示本次执行参数。
...
2. 如果导入 10 万条 CSV，你怎么设计？
3. 如果 Batch 中间失败，怎么避免重复插入？

## Day 13 必回炉建议

1. Spring Batch 是什么？
2. Job 和 Step 是什么？
3. Reader / Processor / Writer 分别是什么？
4. Batch 失败怎么处理？
5. 大文件怎么避免一次性加载？
6. Batch 和普通定时任务区别？
7. JobLauncher / JobRepository 是什么？
8. 接口慢怎么排查？
9. SQL 慢怎么优化？
10. Bean 注入失败怎么排查？

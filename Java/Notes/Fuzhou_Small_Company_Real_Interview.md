...
>
> 只看原题可使用：[福州小厂 Java 面经 1～3（仅题目）](./Fuzhou_Small_Company_Questions_Only.md)。
...
训练分为“基础阶段”和“扩展阶段”，不得跳级。

### 基础阶段：先练完 30 道原题

1. 每天以这 30 道照片原题为主，一次只问一道，不提前展示答案。
2. 题意和难度必须保持与原题一致，不擅自改成源码题或底层原理题。
3. 回答达到“结论正确、要点完整、可以独立口述”即可通过，不要求源码级细节。
4. 回答不完整时，只指出与当前原题直接相关的缺失点，让回答者重新组织；不得借题连续深挖。
5. 回答稳定的题标记为 A；只有关键词、表达不完整标记为 B；原理错误或无法回答标记为 C。
6. B/C 题进入错题本，并在之后 1 天、3 天、7 天按原题重复抽查。
7. 项目题必须结合真实经历，禁止编造生产规模、访问量和个人职责。

### 扩展阶段：30 道全部达标后

1. 每天仍先复习原题和既有错题，再增加少量同层级扩展题。
2. 扩展优先选择常见应用场景和面试官高频追问，每次最多增加 1～2 道。
3. 扩展只到实战级：重点问技术如何选择、如何使用、如何排查问题、如何保证数据和系统安全，以及怎样结合真实项目回答。
4. 不进入源码级追问，不要求背框架调用链、JVM 源码或源码常量；复杂架构题也必须与应聘岗位和真实经历匹配。
5. 扩展题答错时回到对应基础题，先补牢概念，再继续扩展。
